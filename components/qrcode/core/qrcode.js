...
function checkQRVersion(version, sText, nCorrectLevel) {
  ...
  const nLimit = QRCodeLimitLength[i][limitMap[nCorrectLevel]]
  return length <= nLimit
}
function getUTF8Length(sText) {
  const replacedText = encodeURI(sText).replace(/%[0-9a-fA-F]{2}/g, 'a')
...
class QR8bitByte {
  ...
  parseData() {
    ...
    const parsedData = [].concat(...byteArrays)
    if (parsedData.length !== this.data.length) {
      parsedData.unshift(239, 187, 191) // BOM
    }
    return parsedData
  }
  getLength() {
...
export class QRUtil {
  ...
  static getBCHTypeInfo(data) {
    ...
    const result = ((data << 10) | d) ^ QRUtil.G15_MASK
  ...
  static getMask(maskPattern, i, j) {
    const maskFunctions = {
      [QRMaskPattern.PATTERN000]: (i, j) => (i + j) % 2 === 0,
      [QRMaskPattern.PATTERN001]: (i, j) => i % 2 === 0,
      [QRMaskPattern.PATTERN010]: (i, j) => j % 3 === 0,
      [QRMaskPattern.PATTERN011]: (i, j) => (i + j) % 3 === 0,
      [QRMaskPattern.PATTERN100]: (i, j) =>
        (Math.floor(i / 2) + Math.floor(j / 3)) % 2 === 0,
      [QRMaskPattern.PATTERN101]: (i, j) => ((i * j) % 2) + ((i * j) % 3) === 0,
      [QRMaskPattern.PATTERN110]: (i, j) =>
        (((i * j) % 2) + ((i * j) % 3)) % 2 === 0,
      [QRMaskPattern.PATTERN111]: (i, j) =>
        (((i * j) % 3) + ((i + j) % 2)) % 2 === 0,
    }
    const maskFunc = maskFunctions[maskPattern]
    if (!maskFunc) {
  ...
  static getLostPoint(qrCode) {
    ...
    const isDarkCached = (row, col) => {
      const key = `${row},${col}`
      if (!darkCache.has(key)) {
        darkCache.set(key, qrCode.isDark(row, col))
      }
      return darkCache.get(key)
    }
    for (var row = 0; row < moduleCount; row++) {
      for (var col = 0; col < moduleCount; col++) {
        let sameCount = 0
        const dark = isDarkCached(row, col)
        for (let r = -1; r <= 1; r++) {
          ...
          for (let c = -1; c <= 1; c++) {
            ...
            if (dark == isDarkCached(row + r, col + c)) {
              sameCount++
            }
          }
        }
    ...
    for (var row = 0; row < moduleCount - 1; row++) {
      for (var col = 0; col < moduleCount - 1; col++) {
        let count = 0
        if (isDarkCached(row, col)) count++
        if (isDarkCached(row + 1, col)) count++
        if (isDarkCached(row, col + 1)) count++
        if (isDarkCached(row + 1, col + 1)) count++
        if (count == 0 || count == 4) {
          lostPoint += 3
        }
      }
    ...
    for (var row = 0; row < moduleCount; row++) {
      for (var col = 0; col < moduleCount - 6; col++) {
        if (
          isDarkCached(row, col) &&
          !isDarkCached(row, col + 1) &&
          isDarkCached(row, col + 2) &&
          isDarkCached(row, col + 3) &&
          isDarkCached(row, col + 4) &&
          !isDarkCached(row, col + 5) &&
          isDarkCached(row, col + 6)
        ) {
          lostPoint += 40
        }
      }
    ...
    for (var col = 0; col < moduleCount; col++) {
      for (var row = 0; row < moduleCount - 6; row++) {
        if (
          isDarkCached(row, col) &&
          !isDarkCached(row + 1, col) &&
          isDarkCached(row + 2, col) &&
          isDarkCached(row + 3, col) &&
          isDarkCached(row + 4, col) &&
          !isDarkCached(row + 5, col) &&
          isDarkCached(row + 6, col)
        ) {
          lostPoint += 40
        }
      }
...
class QRBitBuffer {
  ...
  _expandCapacity() {
    const newBuffer = new Uint8Array(this.capacity * 2)
